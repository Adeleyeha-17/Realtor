    ...
    <div className="w-full flex max-h-screen">
        <div className="w-full lg:w-1/2 px-6 sm:px-20 py-10 flex flex-col gap-10 relative">
            <div className="flex items-center justify-between">
                ...
                <h2>
                    Not a member?
                    <Link to="/sign-up" className="text-link ml-1 xl:ml-2">Sign up</Link>
                </h2>
            </div>
            <Lottie animationData={Arrow} loop={true} className="absolute right-32 top-20 w-28 -rotate-45"/>

            <div className="flex flex-col gap-2">
                <h1 className="text-4xl font-bold text-primary_head">Sign In</h1>
            ...
            {error && <p className="text-red-600">{error}</p>}
                ...
                <Button disabled={loading}
                    variant={"cta"}
                    className="w-44 h-12 rounded-3xl flex items-center justify-center hover:opacity-95 disabled:opacity-80 transition-all ease-in-out relative mt-4">{loading ? "Loading..." : "Sign in"}
                    <span
